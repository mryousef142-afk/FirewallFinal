...
The project now bundles the private chat entry flow for the management mini app.

1. Copy `.env.example` to `.env` and provide at least `BOT_TOKEN` and `MINI_APP_URL`.
2. Optionally set `BOT_USERNAME`, `CHANNEL_URL`, `ADD_TO_GROUP_URL`, or `START_PAYLOAD`.
3. Start the bot locally with `npm run bot`.

The bot sends the welcome keyboard defined in FW-01, opens the mini app via a `web_app`
button, and answers the other buttons with placeholders so the layout is complete.
Texts and labels are stored in `bot/content.json`; the owner panel can later update this
file or replace the loader implementation.
## Mini App Dashboard (FW-02)

- Landing on the mini app now opens a group management dashboard with a profile header, live counts, and state-aware cards.
- Cards render active, expired, and removed states, respecting the 10-day removal grace window and exposing renew/settings CTAs.
- A Lottie-based empty state (compressed `.tgs` + `pako`) appears when no groups exist, matching the PV invite flow.
- Automatic search becomes available once the operator manages more than 10 groups; counts and labels render with Persian digits.
- All texts and links remain configurable via `.env` (`VITE_INVITE_LINK`, delays) and placeholder callbacks for future backend wiring.

## Group Dashboard (FW-03)

- هر کارت «⚙️ تغییر تنظیمات» حالا یک داشبورد اختصاصی با هدر ثابت، ویجت‌های زنده و منوی ماژول همبرگری باز می‌کند.
- کارت‌های آماری اعضا، اعتبار باقی‌مانده، پیام‌های امروز و اعضای جدید را همراه با برچسب مقایسه با دیروز و CTA تمدید نمایش می‌دهند.
- منوی کشویی مسیرهایی مثل تنظیمات عمومی، ممنوعیت‌ها، محدودیت‌ها، خاموشی‌ها، عضویت اجباری، متن‌ها، آمار پیشرفته، تمدید و گیواوی را در یک لیست قابل اسکرول نگه می‌دارد.

## Ban Rules (FW-05)

- صفحه «ممنوعیت‌ها» مجموعه قوانین لینک، رسانه، زبان و تعامل را با سوئیچ و جدول زمان‌بندی تفکیک می‌کند.
- فیلدهای فهرست کلمات ممنوعه/ضروری با textarea، شمارنده و دکمه‌های وارد کردن/خروجی گرفتن مجهز شده‌اند.
- ذخیره‌سازی قوانين و لیست‌ها به کمک API ساختگی انجام می‌شود و پیام موفقیت (Toast) نمایش داده می‌شود.
